...
public class Scripts {
  ...
  private static void blockstates(String registryname) throws IOException {
    ...
    if (registryname.endsWith("glass"))
      model = "simplecoloredblocks:block/cube_all_tinted_glass";

else

    model = "simplecoloredblocks:block/cube_all_tinted";
  ...
  private static void item(String material) throws IOException {
    ...
    if (material.endsWith("glass"))
      item.addProperty("parent", "simplecoloredblocks:block/cube_all_tinted_glass");

    else item.addProperty("parent", "simplecoloredblocks:block/cube_all_tinted");
  ...
  private static void lang() throws IOException {
    ...
    for (Block block : SimpleColoredBlocks.MOD_BLOCKS){
      if (block.getRegistryName().getPath().endsWith("glass"))
        lang.addProperty("block.simplecoloredblocks."+block.getRegistryName().getPath(),"Red "+((SimpleBlock)block).r + ", Green "+((SimpleBlock)block).g + ", Blue "+((SimpleBlock)block).b+" Glass");

else      lang.addProperty("block.simplecoloredblocks."+block.getRegistryName().getPath(),"Red "+((SimpleBlock)block).r + ", Green "+((SimpleBlock)block).g + ", Blue "+((SimpleBlock)block).b+" Block");
    }
